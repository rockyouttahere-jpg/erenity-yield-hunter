...
import { getAddress as getAddress2, isAddressEqual, parseErc6492Signature as parseErc6492Signature2 } from "viem";

// src/exact/facilitator/eip3009-utils.ts
import { encodeFunctionData, getAddress, parseErc6492Signature, parseSignature } from "viem";
...
async function verifyEIP3009(signer, payload, requirements, eip3009Payload, options) {
  ...
  if (getAddress2(eip3009Payload.authorization.to) !== getAddress2(requirements.payTo)) {
    return {
      isValid: false,
...
async function settleEIP3009(signer, payload, requirements, eip3009Payload, config) {
  ...
  try {
    ...
    const tx = await executeTransferWithAuthorization(
      signer,
      getAddress2(requirements.asset),
      eip3009Payload
    );
...
import { getAddress as getAddress3, encodeFunctionData as encodeFunctionData2 } from "viem";
